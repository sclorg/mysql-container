...
export MYSQL_LOG_FILE=/var/opt/rh/rh-mysql57/log/mysql/mysqld.log
...
# Extracts the initial temporary root password from the log file
function get_autogenerated_root_password() {
  AUTOGENERATED_ROOT_PASSWORD="$(cat $MYSQL_LOG_FILE | grep "temporary password" | awk '{print $NF}')"
}
...
function initialize_database() {
  ...
  if [[ "$MYSQL_VERSION" < "5.7" ]] ; then
    ...
    log_to_file_and_run ${MYSQL_PREFIX}/libexec/mysqld --initialize --datadir=$MYSQL_DATADIR --ignore-db-dir=lost+found
  fi

  get_autogenerated_root_password
  mysql_flags="$mysql_flags -p${AUTOGENERATED_ROOT_PASSWORD}"
  admin_flags="--defaults-file=$MYSQL_DEFAULTS_FILE $mysql_flags"

  start_local_mysql "$@"

  # This workaround is needed to avoid getting an 'expired password' error message
  mysql --connect-expired-password $mysql_flags <<EOSQL
    SET PASSWORD = PASSWORD('${AUTOGENERATED_ROOT_PASSWORD}');
EOSQL
